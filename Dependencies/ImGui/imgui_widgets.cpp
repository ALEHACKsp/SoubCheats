...
bool ImGui::Keybind(const char* str_id, int* current_key, int* key_style) {
    ...
    const ImRect frame_bb(window->DC.CursorPos - ImVec2(str_id_size.x, 0), window->DC.CursorPos + label_size - ImVec2(str_id_size.x, 0));
    const ImRect total_bb(window->DC.CursorPos - ImVec2(str_id_size.x, 0), window->DC.CursorPos + ImVec2(frame_bb.Max.x + style.FramePadding.x - str_id_size.x, label_size.y));
    ...
    if (g.ActiveId == id) {
        ...
        if (key_style) {
            ...
            if (popup_open) {
                ...
                // Horizontally align ourselves with the framed text
                ImGuiWindowFlags window_flags = ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_Popup | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse;
                PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(4, 7));
                bool ret = Begin(name, NULL, window_flags);
                PopStyleVar();
    ...
    ImGui::RenderText(ImVec2(frame_bb.Min.x + style.FramePadding.x + str_id_size.x + 4, frame_bb.Min.y + 3), buf_display);
    if (ImGui::IsItemHovered())
    {
        ImGui::SetTooltip("Click then press a key to bind. Right click to select mode.");
    }
